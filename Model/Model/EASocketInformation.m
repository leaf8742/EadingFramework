#import "EASocketInformation.h"
#import "APManager.h"
#import "Reachability.h"

#if !__has_feature(objc_arc)
#error EASocketInformation must be built with ARC.
// You can turn on ARC for only EASocketInformation.m by adding -fobjc-arc to the build phase.
#endif

#import "Model.h"

@implementation EASocketInformation
...
- (id)init {
    if (self = [super init]) {
        ...
        self.jacks = [NSMutableArray array];
...
- (id)initWithIdentity:(NSString *)identity {
    if (self = [self init]) {
        ...
        self.objectType = kEAObjectTypeSocket;
    }
    return self;
}

- (void)setJackPowerStatWithCode:(NSString *)code {
    for (int i = 0; i < self.jacks.count; i++) {
        NSString *powerCode = [code substringWithRange:NSMakeRange(i, 1)];
        JackInformation *jack = self.jacks[i];
        jack.powerOn = [powerCode isEqualToString:@"1"];
    }
}

- (BOOL)getJackPowerStatWithIndex:(NSInteger)index {
    if (index >= 0 && index < self.jacks.count) {
        JackInformation *jack = self.jacks[index];
        return jack.powerOn;
    }
    return NO;
}
...
- (AttachmentList *)attachmentList {
    if (!_attachmentList) {
        _attachmentList = [[AttachmentList alloc] init];
    }
