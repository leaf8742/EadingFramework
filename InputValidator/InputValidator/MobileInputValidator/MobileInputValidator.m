...
+ (BOOL)validateMobile:(NSString *)mobile {
    if (!mobile) return NO;
    // 正则表达式限制(11位纯数字)
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"^(1[^12][0-9]+)$" options:NSRegularExpressionAnchorsMatchLines error:nil];
    NSUInteger numberOfMatches = [regex numberOfMatchesInString:mobile options:NSMatchingAnchored range:NSMakeRange(0, mobile.length)];
    return numberOfMatches != 0 && [mobile length] == 11;
}
