#import "MinuteNumberInputValidator.h"

@implementation MinuteNumberInputValidator
...
- (BOOL)validateInput:(UITextField *)input shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string error:(NSError **)error {
    ...
    if (result.length > 2 || [result integerValue] > 59 ||
        ([_hour.text integerValue] == 24 && [result integerValue] > 0)) {
        if (error != nil) {
