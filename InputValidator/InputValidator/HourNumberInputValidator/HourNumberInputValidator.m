#import "HourNumberInputValidator.h"

@implementation HourNumberInputValidator
...
- (BOOL)validateInput:(UITextField *)input shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string error:(NSError **)error {
    ...
    if (result.length > 2 || [result integerValue] > 24 ||
        ([result integerValue] == 24 && [_minute.text integerValue] > 0)) {
        if (error != nil) {
